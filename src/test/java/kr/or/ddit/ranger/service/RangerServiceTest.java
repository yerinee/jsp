...
public class RangerServiceTest {
	...
	// 하나의 메소드도 시나리오에 따라서는 여러개의 테스트 메소드르르 생성 할 수도 있다.
	// ex) 회원등록 dao 메소드 생성시
	// 	   정상적인 값을 입력해서 db 테이블에 데이터가 정상적으로 입력되는 성공케이스
	//	   비정상적인 값(회원 id 중복)을 입력하여 db테이블에 데이터가 입력되지 않는 실패 케이스
	...
	// maven 배포시에도 @Test 메소드가 붙은 메소드가 통과되어야 빌드 사이클이 진행이 된다.
	@Test
	public void getRangersTest() {
		...
		/***Then : 결과 ***/
		assertNotNull(rangers); //null이 아니어야 통과
